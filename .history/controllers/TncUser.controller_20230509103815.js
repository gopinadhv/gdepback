...
var fs = require('fs');
const generatePdf = require('../config/pdfGenerate')
...
const genPdf = async function (req, res) {
    ...
    if (body.id) {
        ...
        if (activeData) {
            var jsonEmailData = JSON.stringify(activeData);
            var jsondata = JSON.parse(jsonEmailData);
            let name = `                 ${jsondata.firstName}  ${jsondata.lastName} `;
            ...
            if (jsondata.clientType == 1) {
                ...
                var filename = 'public/pdfs/' + jsondata.firstName + '_' + jsondata.id + '_' + 'tc.pdf';


                var docDefinitions = generatePdf.docDefinition(jsondata.clints, name);
                ...
                var TncList = generatePdf.TncList(name);
                var pdfDoc = printer.createPdfKitDocument(TncList);
