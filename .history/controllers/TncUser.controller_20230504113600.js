...
// const g = require('../views/images/tc.png')
...
const sendEmailUser = async function (req, res) {
    ...
    if (activeData && activeData.length) {
        ...
        for (let x = 0; x < jsondata.length; x++) {
            ...
            var encriptedEmailBe = ''
            const salts = 12; // Number of salt rounds for bcrypt
            encriptedEmailBe = await bcrypt.hash(element.email, salts);
            element.genarateLink = encriptedEmailBe
            ...
            authService.expDateMail(mailData, async function (data) {
                if (data.success == true) {
                    ...
                    data["tcLink"] = element.genarateLink
                    data["linkStatus"] = 1;
                    let tncLinkData;
                    [err, tncLinkData] = await to(Tnc_UserEmails.create(data));
...
const sendPasswordandURLCheck = async function (req, res) {
    ...
    if (body.encriptedEmail) {
                ...
                include: [
                    {
                        ...
                        as: "user",
                        attributes: ["id", "userId", "tcAcceptorDeclineStatus"],
                        where: {
        ...
        if (activeData) {
            var jsondata = JSON.parse(JSON.stringify(activeData));
            if ((jsondata.user && jsondata.user.tcAcceptorDeclineStatus == 0)) {
                ...
                return res.status(200).json({ msg: false, error: 'Already Accept TC', email: jsondata.email, data: jsondata })
            ...
            if (jsondata && jsondata.passwordTC) {
                bcrypt.compare(body.tncPassword, jsondata.passwordTC, async function (err, results) {
                    ...
                    if (results) {
                        ...
                        return res.status(200).json({ msg: true, clints: jsondata.clints ? jsondata.clints : '', data: jsondata })
                    } else {
        ...
        `;
                ...
                phantom.create().then(function (ph) {
                    ph.createPage({ format: 'A4', orientation: 'portrait', }).then(function (page) {
                        ...
                        const headerHtml = `
      <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100px;"> text
        // <img src="../views/images/tc.png" style="max-width: 100%; max-height: 100%;">
      </div>
    `;

                        // Set the header HTML as a PhantomJS page template
                        page.property('header', headerHtml);

                        page.property('paperSize', { format: 'A4', orientation: 'portrait', margin: '0.5cm' });
...
const genPdf = async function (req, res) {



    var d = new Date();
    var n = d.valueOf();
    var filename = 'public/pdfs/' + n + 'quotation.pdf';

    var Title = 'Title: Quotation'
    var docDefinitions = generatePdf.docDefinition(activeData, pdfData);
    var pdfDoc = printer.createPdfKitDocument(docDefinitions);

    pdfDoc.pipe(fs.createWriteStream(filename)).on('finish', function () {
        var mailData = {};
        mailData['html'] = 'Text';
        mailData['subject'] = 'Sample mail';
        mailData['to'] = bodyData.tomailId;
        mailData['cc'] = bodyData.ccmailId;
        mailData['qId'] = req.params.id;
        mailData['filename'] = filename;
        mailData['quotationNumber'] = activeData.quotationNumber;
        authService.samplAttachMail(mailData);

        let body = {};
        body['quotationEmailStatus'] = 1;
        QuotationsDetails.find({
            where: {
                id: mailData.qId
            }
        }).then(qoData => {
            return qoData.updateAttributes(body)
        })
            .then(updateStatus => {
    ...
    pdfDoc.end();
    return ReS(res, { data: 'Mail sent Sucessfully' });
}
  else {
    return ReS(res, { data: 'No Data Found' });
}
};
